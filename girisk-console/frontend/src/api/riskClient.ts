...
export const riskApi = {
  ...
  decisionsByFixture: (fixtureId: string, limit = 100) =>
    request<import('../types').RiskDecisionLog[]>(
      `/decisions/by-fixture/${encodeURIComponent(fixtureId)}?limit=${limit}`,
    ),
  ...
  dorisAuditConfig: () =>
    request<import('../types').DorisAuditConfigView>('/audit/doris'),
  putDorisAuditConfig: (body: import('../types').DorisAuditConfigRequest) =>
    request<import('../types').DorisAuditConfigView>('/audit/doris', {
      method: 'PUT',
      body: JSON.stringify(body),
    }),
  testDorisAuditConfig: (body: import('../types').DorisAuditConfigRequest) =>
    request<{ ok: boolean; message: string }>('/audit/doris/test', {
      method: 'POST',
      body: JSON.stringify(body),
    }),
