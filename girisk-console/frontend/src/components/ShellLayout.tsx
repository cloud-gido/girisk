...
import TimezoneHeaderControl from './TimezoneHeaderControl';
...
export default function ShellLayout({
  ...
  const appearance = prefs.appearance || 'classic';

  return (
    <Layout
      style={{ minHeight: '100vh' }}
      className={`shell density-${prefs.density} theme-${appearance}`}
    >      <Sider
        width={220}
        theme="light"
        style={{
      ...
      >
        <div style={{ padding: '18px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
          <BrandMark size={34} />
          <div>
            <div style={{ fontSize: 15, fontWeight: 650, letterSpacing: '-0.02em', color: 'var(--shell-accent, #134e4a)' }}>
              {productName}
            </div>
