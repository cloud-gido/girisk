...
export default function LiabilityBoardPage() {
  ...
  /** KPI 下钻到赛事工作台（保留球类/联赛上下文，覆盖状态类筛选）。 */
      ...
      content: (
        <div>
          ...
          <ul style={{ paddingLeft: 20, marginBottom: 8 }}>
            <li>Redis 赛事物化视图与高危榜</li>
            <li>ExposureStore 盘口占用</li>
            <li>sports_match 全部行</li>
            <li>单场限额/门控覆盖</li>
          </ul>
          <p style={{ marginBottom: 0 }}>
            不会删除 overall / 球类 / 联赛层配置。此操作不可撤销，仅管理员可用。
          </p>
        </div>
      ),
      onOk: async () => {
        ...
        try {
          const r = await sportsApi.purgeAllFixtures();
          message.success(
            `已清除赛事 ${String(r.matchCodes ?? 0)} 场（库 ${String(r.sportsMatchDeleted ?? 0)}）`,
          );
          setExpandedMatchCodes([]);
          setExpandCache({});
          setDrawerCode(null);
          await refresh();
        } catch (e) {
        ...
        <div className="liability-chrome-main">
          ...
          {admin ? (
            <Space wrap className="liability-chrome-actions">
              <Tooltip title="清除 Redis 视图与 sports_match 测试数据（不可撤销）">
                <Button
                  size="small"
                  danger
                  loading={purgeLoading}
                  onClick={purgeAllFixtures}
                >
                  一键清除赛事
                </Button>
              </Tooltip>
...
function MatchInlineMonitor({
  ...
  const [segmentTab, setSegmentTab] = useState<'all' | 'pre' | 'live'>('all');
  const [segmentLimits, setSegmentLimits] = useState<{
    delta?: number;
  ...
  useEffect(() => {
    let cancelled = false;
    setSegmentLimits(null);
    void (async () => {
      try {
        const v = await sportsApi.getLimitOverride(matchCode, segmentTab);
        if (cancelled) return;
        setSegmentLimits({
        ...
        if (!cancelled) setSegmentLimits(null);
      }
    })();
    return () => {
      cancelled = true;
    };
  }, [matchCode, segmentTab]);
  ...
  const matchWithSegmentLimits: SportsMatchView = segmentLimits
    ? {
        ...view,
        delta: segmentLimits.delta ?? view.delta,
        seedPayoutYuan: segmentLimits.seedPayoutYuan ?? view.seedPayoutYuan,
        maxWorstLossYuan: segmentLimits.maxWorstLossYuan ?? view.maxWorstLossYuan,
        maxBetPayoutYuan: segmentLimits.maxBetPayoutYuan ?? view.maxBetPayoutYuan,
      }
    : view;
  const resolved = resolveSegmentViews(matchWithSegmentLimits, cache.fixture, segmentTab);
  ...
  return (
    <div className="liability-inline-panel" onClick={(e) => e.stopPropagation()} style={{ padding: '4px 8px 12px' }}>
      ...
      <Tabs
        ...
        onChange={(k) => setSegmentTab(k as 'all' | 'pre' | 'live')}
      ...
      {resolved.empty ? (
        <Empty
          image={Empty.PRESENTED_IMAGE_SIMPLE}
          description={segmentTab === 'pre' ? '暂无赛前订单' : '暂无滚球订单'}
          style={{ margin: '16px 0' }}
        />
      ) : (
        <>
          <MatchInterceptSummary liveView={resolved.fixture} match={resolved.match} />
          <div style={{ marginTop: 8 }}>
            <MatchMarketsCollapse view={resolved.match} />
          </div>
        </>
      )}
    </div>
...
type SegmentTab = 'all' | 'pre' | 'live';

function resolveSegmentViews(
  match: SportsMatchView,
  fixture: RiskFixtureView | null | undefined,
  tab: SegmentTab,
): { match: SportsMatchView; fixture: RiskFixtureView | null | undefined; empty: boolean } {
  if (tab === 'all') {
    return { match, fixture, empty: false };
  }
  const seg = (fixture?.segments?.[tab] || null) as FixtureSegmentView | null;
  const groups = mapSegmentMarketGroups(seg?.marketGroups);
  const replayStats = (seg?.replayStats || null) as FixtureReplayStats | null;
  const accepted = Number(replayStats?.acceptedCount ?? 0);
  const rejected = Number(replayStats?.rejectedTotal ?? 0);
  const duplicate = Number(replayStats?.duplicateCount ?? 0);
  const total = Number(replayStats?.totalOrders ?? accepted + rejected + duplicate);
  const poolCount = Number(
    seg?.confirmedOrders ?? replayStats?.confirmedPoolCount ?? 0,
  );
  const hasExposure =
    groups.some((g) => (g.outcomes || []).some((o) => Number((o as OutcomeLimitRow & { actualStake?: number }).actualStake ?? 0) > 0))
    || Number(replayStats?.heldRiskOrderCount ?? 0) > 0
    || poolCount > 0;
  const empty =
    Boolean(seg?.empty) && !hasExposure && total === 0 && groups.length === 0 && !seg?.replayStats;
  if (empty) {
    return { match, fixture, empty: true };
  }
  // 段视图禁止回退到 ALL 的 worst* / confirmedOrders，避免赛前串到整场盈亏
  const segmentWorstCents =
    seg?.worstLossCents != null
      ? Number(seg.worstLossCents)
      : replayStats?.withRiskWorstPnlYuan != null
        ? Math.round(Math.abs(Number(replayStats.withRiskWorstPnlYuan)) * 100)
        : 0;
  const segmentWorstScore =
    seg?.worstScore
    || replayStats?.withRiskWorstScore
    || undefined;
  const syntheticFixture: RiskFixtureView | null | undefined = fixture
    ? {
        ...fixture,
        confirmedOrders: poolCount,
        worstLossCents: segmentWorstCents,
        worstScore: segmentWorstScore,
        replayStats: replayStats
          ? {
              ...replayStats,
              // 段内最差：只用本段，不拼 ALL
              withRiskWorstPnlYuan:
                replayStats.withRiskWorstPnlYuan
                ?? (segmentWorstCents ? -Math.abs(segmentWorstCents) / 100 : undefined),
              withRiskWorstScore: replayStats.withRiskWorstScore ?? segmentWorstScore,
            }
          : {
              acceptedCount: accepted,
              acceptedStakeYuan: 0,
              totalOrders: total,
              confirmedPoolCount: poolCount,
            },
        marketGroups: groups,
      }
    : fixture;
  return {
    match: { ...match, marketGroups: groups },
    fixture: syntheticFixture,
    empty: false,
  };
}

function mapSegmentMarketGroups(
  raw: FixtureSegmentView['marketGroups'] | undefined,
): MarketGroupView[] {
  if (!raw || !Array.isArray(raw) || raw.length === 0) {
    return [];
  }
  const out: MarketGroupView[] = [];
  for (const g of raw) {
    ...
    const outcomes: OutcomeLimitRow[] = [];
    for (const o of outcomesRaw) {
      if (!o || typeof o !== 'object') continue;
      const om = o as Record<string, unknown>;
      const selection = String(om.selection || '');
      if (!selection) continue;
      const stake = Number(om.stake) || 0;
      const actualStake =
        om.actualStake != null && om.actualStake !== ''
          ? Number(om.actualStake)
          : undefined;
      const seedYuan =
        om.seedYuan != null && om.seedYuan !== '' ? Number(om.seedYuan) : undefined;
      const targetAmount = Number(om.targetAmount) || 0;
      const maxAllowedAmount = Number(om.maxAllowedAmount) || 0;
      const acceptMax = Number(om.acceptMax) || 0;
      stakes[selection] = stake;
      limits[selection] = acceptMax;
      outcomes.push({
        selection,
        stake,
        actualStake,
    ...
    out.push({
      ...
      marketLabel: String(row.marketLabel || row.marketType || ''),
