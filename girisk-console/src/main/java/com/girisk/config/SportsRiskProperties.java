...
public class SportsRiskProperties {
    ...
    /** 冷启动种子（返彩元）；与 Engine limit.seedPayoutYuan / 指标卡「初始已投注」默认对齐。 */
    private double seedPayoutYuan = 5000;
    /** 最差亏损阈值（元）；与 Engine exposure.maxWorstLossYuan / 指标卡「风险阈值」默认对齐。 */
    private double maxWorstLossYuan = 200000;
