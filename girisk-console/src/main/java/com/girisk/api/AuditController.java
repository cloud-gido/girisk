...
public class AuditController {
    ...
    @GetMapping("/decisions/by-fixture/{fixtureId}")
    public ApiResponse<List<RiskDecisionLog>> decisionsByFixture(
            @PathVariable String fixtureId,
            @RequestParam(defaultValue = "100") int limit) {
        List<RiskDecisionLog> logs = decisionLogRepository.findByFixtureId(fixtureId, limit);
        if (tenantEnforce) {
            String op = tenantContext.getOperatorId();
            if (op != null) {
                logs = logs.stream().filter(l -> op.equals(l.operatorId())).toList();
