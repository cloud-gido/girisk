...
/**
 * 限额分层：赛前/滚球 → 赛事 → 联赛 → 球类 → 总体 → 全局默认。
 * 整场赛事覆盖仍写入 {@code girisk:override:fixture:*}；赛前/滚球写入 scope store（MATCH_PRE/LIVE）。
 */
@Service
public class ScopeLimitParamsService {
    ...
    public ScopeLimitParamsView getView(LimitScopeType type, String scopeKey) {
        ...
        if (type == LimitScopeType.MATCH) {
            throw new BusinessException("赛事请使用 /matches/{code}/limit-override?segment=all");
        }
        if (type == LimitScopeType.MATCH_PRE || type == LimitScopeType.MATCH_LIVE) {
            throw new BusinessException("赛前/滚球请使用 /matches/{code}/limit-override?segment=pre|live");
        }
    ...
    public ScopeLimitParamsView upsert(LimitScopeType type, String scopeKey, FixtureLimitOverrideRequest req) {
        if (type == LimitScopeType.MATCH
                || type == LimitScopeType.MATCH_PRE
                || type == LimitScopeType.MATCH_LIVE) {
            throw new BusinessException("赛事分段请使用 /matches/{code}/limit-override?segment=all|pre|live");
        }
    ...
    /**
     * 解析某场有效参数：全局默认 ← 总体 ← 球类 ← 联赛 ← 赛事覆盖（整场）← 可选赛前/滚球。
     * {@code sports_match} 库内阈值不作静默默认（避免与 properties 分叉）；仅 fixture/scope 覆盖生效。
     */
    public Resolved resolveForMatch(SportsMatch match) {
        return resolveForMatch(match, MatchLimitSegment.ALL);
    }

    /**
     * @param segment {@link MatchLimitSegment#PRE}/{@link MatchLimitSegment#LIVE} 时在 MATCH 之上再叠段覆盖
     */
    public Resolved resolveForMatch(SportsMatch match, MatchLimitSegment segment) {
        ...
        // 不把 sports_match.exposure_threshold / delta 当作静默「赛事默认」叠进解析：
        // 否则库内历史值（如 200000）与 properties/CLI（如 1000）分叉，整场 Tab 与赛前/滚球不一致。
        // 显式覆盖仅来自 fixture / scope override（MATCH、MATCH_PRE、MATCH_LIVE）。
        ...
        ScopeLimitOverride matchOv = fixtureStore.get(match.matchCode())
                .map(f -> new ScopeLimitOverride(
                        ...
                        f.updatedAt()))
                .orElse(null);
        Layer afterMatch = apply(layer, matchOv);

        ScopeLimitOverride segmentOv = null;
        Layer effective = afterMatch;
        if (seg == MatchLimitSegment.PRE || seg == MatchLimitSegment.LIVE) {
            LimitScopeType st = seg.scopeType();
            segmentOv = scopeStore.get(st, match.matchCode()).orElse(null);
            effective = apply(afterMatch, segmentOv);
        }

        boolean active = seg == MatchLimitSegment.ALL
                ? (matchOv != null && matchOv.hasAny())
                : (segmentOv != null && segmentOv.hasAny());
        ScopeLimitOverride reported = seg == MatchLimitSegment.ALL ? matchOv : segmentOv;
        return new Resolved(
                effective.delta, effective.seed, effective.maxWorst, effective.maxBet, active, reported);
    }

    /** 写入赛前/滚球 scope 覆盖（由 FixtureLimitParamsService 调用）。 */
    public void putMatchSegmentOverride(LimitScopeType type, String matchCode, ScopeLimitOverride override) {
        if (type != LimitScopeType.MATCH_PRE && type != LimitScopeType.MATCH_LIVE) {
            throw new BusinessException("仅支持 MATCH_PRE / MATCH_LIVE");
        }
        String key = normalizeKey(type, matchCode);
        if (override == null || !override.hasAny()) {
            scopeStore.delete(type, key);
            log.info("Cleared scope limit override {}/{}", type, key);
        } else {
            scopeStore.put(new ScopeLimitOverride(
                    type,
                    key,
                    override.delta(),
                    override.seedPayoutYuan(),
                    override.maxWorstLossYuan(),
                    override.maxBetPayoutYuan(),
                    override.updatedBy(),
                    override.updatedAt() != null ? override.updatedAt() : Instant.now()));
            log.info("Saved scope limit override {}/{} by={}", type, key, override.updatedBy());
        }
        configDispatch.afterScopeWrite(type, key);
    }

    public Optional<ScopeLimitOverride> getMatchSegmentOverride(LimitScopeType type, String matchCode) {
        if (type != LimitScopeType.MATCH_PRE && type != LimitScopeType.MATCH_LIVE) {
            return Optional.empty();
        }
        return scopeStore.get(type, normalizeKey(type, matchCode));
    }
