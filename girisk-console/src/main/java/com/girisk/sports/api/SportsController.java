...
import com.girisk.sports.service.FixtureViewPurgeService;
...
public class SportsController {
    ...
    private final FixtureViewPurgeService fixtureViewPurgeService;
    ...
    public SportsController(
            ...
            FixtureViewPurgeService fixtureViewPurgeService,
        ...
        this.fixtureViewPurgeService = fixtureViewPurgeService;
    ...
    /** 场次有效限额参数（覆盖 ⊕ 赛事库 ⊕ 全局）。segment=all|pre|live，默认 all。 */
    @GetMapping("/matches/{matchCode}/limit-override")
    public ApiResponse<FixtureLimitParamsView> getLimitOverride(
            @PathVariable String matchCode,
            @RequestParam(defaultValue = "all") String segment) {
        return ApiResponse.ok(limitParamsService.getView(matchCode, MatchLimitSegment.from(segment)));
    }

    /**
     * 交易员场次级限额覆盖：δ / 种子 / 最差亏损阈值 / 单注返彩上限。
     * segment=all 写整场；pre/live 写赛前/滚球（叠在整场之上）。
     * 字段传 null 表示该项不覆盖；全部 null 等同清除。
     */
    ...
    public ApiResponse<FixtureLimitParamsView> putLimitOverride(
        ...
        return ApiResponse.ok(limitParamsService.upsert(matchCode, body, MatchLimitSegment.from(segment)));
    }

    @DeleteMapping("/matches/{matchCode}/limit-override")
    public ApiResponse<FixtureLimitParamsView> clearLimitOverride(
            @PathVariable String matchCode,
            @RequestParam(defaultValue = "all") String segment) {
        return ApiResponse.ok(limitParamsService.clear(matchCode, MatchLimitSegment.from(segment)));
    }
     ...
     * {@code force=true} 时覆盖已有高危表数据。生产看板已隐藏入口，仅保留 API 供本地联调。
     */
    ...
    /**
     * 一键清除全部赛事测试数据（Redis 视图 + ExposureStore + sports_match + 单场覆盖）。
     * 仅 ROLE_ADMIN；不删除 overall/sport/league 层配置。
     */
    @PostMapping("/fixtures/purge")
    @PreAuthorize("hasRole('ADMIN')")
    public ApiResponse<Map<String, Object>> purgeAllFixtures() {
        dutyAuth.requireAdmin();
        return ApiResponse.ok(fixtureViewPurgeService.purgeAllMatches());
    }
