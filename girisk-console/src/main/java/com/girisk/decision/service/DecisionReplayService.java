...
public class DecisionReplayService {
    ...
    private final DorisAuditProperties dorisAuditProperties;
    ...
    public DecisionReplayService(
            ...
            ObjectProvider<DorisAuditProperties> dorisAuditProperties,
            ObjectProvider<DorisAuditQueryRepository> dorisAuditQueryRepository) {
        ...
        this.dorisAuditProperties = dorisAuditProperties.getIfAvailable();
        this.dorisAuditQueryRepository = dorisAuditQueryRepository.getIfAvailable();
    }
    ...
    public Map<String, Object> replayByOrderId(String orderId) {
        if (dorisEnabled()) {
            try {
                ...
                log.warn("Doris replay by order failed, fallback MySQL: {}", e.getMessage());
            }
        }
    ...
    public Map<String, Object> replayByTraceId(String traceId) {
        if (dorisEnabled()) {
            try {
                ...
                log.warn("Doris replay by trace failed, fallback MySQL: {}", e.getMessage());
            }
        }
    ...
    private boolean dorisEnabled() {
        return dorisAuditProperties != null
                && dorisAuditProperties.isEnabled()
                && dorisAuditQueryRepository != null
                && dorisAuditQueryRepository.available();
    }
