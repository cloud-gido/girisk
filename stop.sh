...
stop_docker() {
  # 默认只停 Console 容器，保留 redis/kafka/postgres 以便本地演示
...
if [[ "${STACK}" == "true" ]]; then
  stop_local || true
  pkill -f 'girisk-console-1.0.0.jar' 2>/dev/null || true
  if command -v docker >/dev/null 2>&1 && [[ -f docker-compose.yml ]]; then
    docker compose --profile doris down
    echo "已停止全栈（Console + PostgreSQL/Kafka/Redis/Doris）"
  else
    echo "已停止 Console；Docker 不可用或无 compose 文件"
  fi
  exit 0
fi
